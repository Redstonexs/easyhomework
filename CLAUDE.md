...
- **Model capability detection** (vision / function-calling / thinking) lives in `LLMRepository.fetchModels()` (reads `/v1/models` metadata) with name-based fallback in `LLMConfig.modelSupportsVision()` / `modelSupportsFunctionCalling()`. `CapabilitySource` records whether a capability came from the API, was auto-guessed, or manually overridden; `LLMConfig.supportsVisionInput()` resolves the final answer at send time (and re-derives via `modelSupportsVision` for the `AUTO` source, so name heuristics must stay correct there too).
